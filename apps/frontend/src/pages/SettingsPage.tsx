...
export default function SettingsPage() {
  ...
  const visibleProviders = useMemo(() => {
    return KNOWN_PROVIDERS.filter((p) => {
      const isSavedActive = llmConfigs.some(
        (c) => c.provider_name === p.name && c.is_active,
      );
  ...
  const visibleServers = useMemo(() => {
    return KNOWN_SERVERS.filter((s) => {
      const isSavedActive = mcpConfigs.some(
        (c) => c.server_name === s.name && c.is_active,
      );
  ...
  const availableProvidersToAdd = KNOWN_PROVIDERS.filter(
    (p) => !visibleProviders.includes(p),
  );
  const availableServersToAdd = KNOWN_SERVERS.filter(
    (s) => !visibleServers.includes(s),
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <p className="text-muted-foreground mb-8">
        ...
        and LLM Providers. These credentials will be securely stored and used
        only during your own kit executions. If left empty, the global system
        credentials will be used as a fallback if available.
      </p>
      ...
      <div className="space-y-8">
        ...
        <div className="card p-6">
          ...
          <div className="space-y-4 max-w-2xl mt-4">
            <div>
              <label className="block text-sm font-medium mb-1.5 font-mono text-xs text-muted-foreground">
                Email Address
              </label>
        ...
        {visibleProviders.length === 0 ? (
          ...
          visibleProviders.map((provider) => {
            const existingConfig = llmConfigs.find(
            ...
            return (
              <LlmProviderForm
                ...
                onSave={(envVars, selectedModel, isActive) =>
                  handleSaveLlm(provider.name, envVars, selectedModel, isActive)
                }
              />
            );
          })
          ...
          visibleServers.map((server) => {
            ...
            return (
              <ServerConfigForm
                ...
                onSave={(envVars, isActive) =>
                  handleSaveMcp(server.name, envVars, isActive)
                }
              />
            );
          })
...
function LlmProviderForm({
  ...
  const showSecret = (v: string) =>
    setVisibleVars((prev) => ({ ...prev, [v]: true }));
  const hideSecret = (v: string) =>
    setVisibleVars((prev) => ({ ...prev, [v]: false }));
  ...
  return (
    <div className="card p-6">
      ...
      <div className="space-y-4 max-w-2xl">
        <div>
          <label className="block text-sm font-medium mb-1.5 font-mono text-xs">
            Default Model
          </label>
          <select
          ...
          <div key={v}>
            ...
            {v === "GOOGLE_APPLICATION_CREDENTIALS_JSON" ? (
              <textarea
                ...
                value={formVars[v] || ""}
                onChange={(e) =>
                  handleFormChange({ ...formVars, [v]: e.target.value })
                }

...
import { getKit, startExecution, getSSEUrl, submitEvaluation, pauseExecution, resumeExecution, formatToolName, type Resource, type Tool } from '../lib/api';
                            ...
                            <div className="flex flex-wrap gap-2">
                                {kitTools.map((t) => (
                                    <span key={t.tool_id} className="badge" style={{ background: 'var(--color-muted)', borderColor: 'var(--color-border)', color: 'var(--color-foreground)' }} title={t.tool_name}>
                                        ...
                                        {t.display_name || formatToolName(t.tool_name)}
                                        {t.source && t.source !== 'builtin' && (
                                            <span className="badge badge-outline text-[10px] ml-1.5" style={{ opacity: 0.7 }}>{t.source}</span>
                                        )}
                                    </span>
                                ))}
                            </div>
